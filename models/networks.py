...
class SpatialAttention(nn.Module):
    ...
    def __init__(self, kernel_size=7):
        """Initialize the Spatial Attention layer with a convolutional layer.

        Parameters:
            kernel_size (int) -- size of the convolutional kernel (default: 7)
        """
    ...
    def forward(self, x):
        """Forward function for spatial attention.

        Parameters:
            x (tensor) -- input feature maps

        Returns the feature maps weighted by the spatial attention.
        """
...
class ResnetGenerator(nn.Module):
    """Resnet-based generator that consists of Resnet blocks with additional features."""

    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6, padding_type='reflect'):
        """Construct a Resnet-based generator with dilated convolutions, spatial attention, and Leaky ReLU.
        ...
        mult = 2 ** n_downsampling
        for i in range(n_blocks):  # add ResNet blocks with new features
            dilation = 2 if i % 2 == 0 else 1  # Apply dilation to every other block
            use_spatial_attention = (i % 3 == 0)  # Apply spatial attention to every third block
            use_leaky_relu = (i % 2 == 0)  # Use Leaky ReLU in some blocks
            model += [ResnetBlock(ngf * mult, padding_type=padding_type, norm_layer=norm_layer, 
                                  use_dropout=use_dropout, use_bias=use_bias, dilation=dilation, 
                                  use_spatial_attention=use_spatial_attention, use_leaky_relu=use_leaky_relu)]
...
class ResnetBlock(nn.Module):
    """Define a ResNet block with dilated convolutions, optional spatial attention, and Leaky ReLU."""

    def __init__(self, dim, padding_type, norm_layer, use_dropout, use_bias, dilation=1, use_spatial_attention=False, use_leaky_relu=False):
        """Initialize the Resnet block with additional features.
        ...
        Parameters:
            ...
            use_dropout (bool)         -- if use dropout layers
            use_bias (bool)            -- if the conv layer uses bias or not
            dilation (int)             -- dilation rate for dilated convolutions
            use_spatial_attention (bool)-- if True, adds spatial attention after the conv block
            use_leaky_relu (bool)      -- if True, uses Leaky ReLU instead of ReLU
        """
        super(ResnetBlock, self).__init__()
        self.use_spatial_attention = use_spatial_attention

        self.conv_block = self.build_conv_block(dim, padding_type, norm_layer, use_dropout, use_bias, dilation, use_leaky_relu)

        if self.use_spatial_attention:
            self.spatial_attention = SpatialAttention()

    def build_conv_block(self, dim, padding_type, norm_layer, use_dropout, use_bias, dilation, use_leaky_relu):
        """Construct a convolutional block with optional dilation and Leaky ReLU.
        ...
        Returns a conv block with optional features.
        """
        conv_block = []
        p = 0
        if padding_type == 'reflect':
        ...
        if use_leaky_relu:
            conv_block += [nn.LeakyReLU(0.2, True)]
        else:
        ...
        conv_block += [nn.Conv2d(dim, dim, kernel_size=3, padding=p, dilation=1, bias=use_bias),
                       norm_layer(dim)]

        return nn.Sequential(*conv_block)

    def forward(self, x):
        """Forward function with skip connection and optional spatial attention.

        Parameters:
            x (tensor) -- input tensor

        Returns the tensor after passing through the ResNet block.
        """
