...
# Assuming `regnet_y_400mf` is available as a function to load the model
class IHCClassifier(nn.Module):
    def __init__(self, num_classes=2):
        super(IHCClassifier, self).__init__()
        self.regnet = models.regnet_y_400mf(pretrained=True)  # Or your function to load RegNet
        num_ftrs = self.regnet.fc.in_features
        self.regnet.fc = nn.Linear(num_ftrs, num_classes)  # Replace the classifier
...
def train(model, dataloader, optimizer, criterion, device):
    ...
    torch.save(model, "./saved_models/regnet_y_400mf")
